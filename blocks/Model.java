...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        if (row + piece.height() > _height || col + piece.width() > _width) {
            return false;
        }
    ...
    boolean placeable(Piece piece) {
        ...
        for (int r = 0; r <= _height - piece.height(); r++) {
            for (int c = 0; c <= _width - piece.width(); c++) {
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int clearScore = (nrows * _width) + (ncols * (_height - nrows));
        int bonusScore = _streakLength * ((nrows * _width) + (ncols * _width));
        return  clearScore + bonusScore;
    }
